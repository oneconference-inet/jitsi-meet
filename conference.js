...
export default {
  ...
  async mutePresenter(mute, showUI = true) {
    ...
    if (mute) {
      ...
      try {
        await this.localVideo.setEffect(
          await this._createPresenterStreamEffect()
        );
      } catch (err) {
  ...
  muteVideo(mute, showUI = true) {
    ...
    if (this.isSharingScreen) {
      ...
      _prevMutePresenterVideo = _prevMutePresenterVideo.then(() =>
        this._mutePresenterVideo(mute)
      );
  ...
  useVideoStream(newTrack) {
    return new Promise((resolve, reject) => {
      _replaceLocalVideoTrackQueue.enqueue((onFinish) => {
        ...
        APP.store
          .dispatch(replaceLocalTrack(this.localVideo, newTrack, room))
          .then(() => {
  ...
  _setSharingScreen(newStream) {
    ...
    if (wasSharingScreen !== this.isSharingScreen) {
      ...
      APP.API.notifyScreenSharingStatusChanged(this.isSharingScreen, details);
    }
  },
  ...
  useAudioStream(newTrack) {
    return new Promise((resolve, reject) => {
      _replaceLocalAudioTrackQueue.enqueue((onFinish) => {
        ...
        APP.store
          .dispatch(replaceLocalTrack(this.localAudio, newTrack, room))
          .then(() => {
  ...
  async _turnScreenSharingOff(didHaveVideo) {
    ...
    if (didHaveVideo) {
      promise = promise
        .then(() => createLocalTracksF({ devices: ["video"] }))
  ...
  async toggleScreenSharing(
    toggle = !this._untoggleScreenSharing,
    options = {}
  ) {
  ...
  async _createPresenterStreamEffect(height = null, cameraDeviceId = null) {
    if (!this.localPresenterVideo) {
      const camera =
        cameraDeviceId ?? getUserSelectedCameraDeviceId(APP.store.getState());

      try {
        this.localPresenterVideo = await createLocalPresenterTrack(
    ...
    try {
      const effect = await createPresenterEffect(
  ...
  async _mutePresenterVideo(mute) {
    ...
    if (
      mute &&
      (!this.localPresenterVideo || this.localPresenterVideo.isMuted())
    ) {
      return;
    } else if (
      !mute &&
      this.localPresenterVideo &&
      !this.localPresenterVideo.isMuted()
    ) {
      return;
    }
    ...
    if (!this.localPresenterVideo && !mute) {
      const { height, width } =
        this.localVideo.track.getSettings() ??
        this.localVideo.track.getConstraints();
      const isPortrait = height >= width;
      const DESKTOP_STREAM_CAP = 720;

      // Config.js setting for resizing high resolution desktop tracks to 720p when presenter is turned on.
      const resizeEnabled =
        config.videoQuality && config.videoQuality.resizeDesktopForPresenter;
      ...
      const resizeDesktopStream =
        browser.isFirefox() || (highResolutionTrack && resizeEnabled);
      ...
      if (resizeDesktopStream) {
        ...
        try {
          await this.localVideo.track.applyConstraints(
            desktopResizeConstraints
          );
        } catch (err) {
          logger.error(
  ...
  _switchToScreenSharing(options = {}) {
    ...
    return this._createDesktopTrack(options)
      .then(async (streams) => {
        const desktopVideoStream = streams.find(
          (stream) => stream.getType() === MEDIA_TYPE.VIDEO
        );
        ...
        this._desktopAudioStream = streams.find(
  ...
  _setupListeners() {
    ...
    room.on(JitsiConferenceEvents.PARTCIPANT_FEATURES_CHANGED, (user) =>
      APP.UI.onUserFeaturesChanged(user)
    );
    ...
    room.on(JitsiConferenceEvents.TRACK_AUDIO_LEVEL_CHANGED, (id, lvl) => {
      ...
      if (this.isLocalId(id) && this.localAudio && this.localAudio.isMuted()) {
        newLvl = 0;
      }
    ...
    room.on(JitsiConferenceEvents.DISPLAY_NAME_CHANGED, (id, displayName) => {
      ...
      APP.API.notifyDisplayNameChanged(id, {
        ...
        formattedDisplayName: appendSuffix(
          formattedDisplayName || interfaceConfig.DEFAULT_REMOTE_DISPLAY_NAME
        ),
      });
    ...
    room.addCommandListener(this.commands.defaults.AVATAR_URL, (data, from) => {
      APP.store.dispatch(
        participantUpdated({
          conference: room,
          id: from,
    ...
    APP.UI.addListener(UIEvents.AUDIO_DEVICE_CHANGED, (micDeviceId) => {
      ...
      createLocalTracksF({
        ...
        .then(() => {
          if (hasDefaultMicChanged) {
            ...
            this.localAudio._realDeviceId = this.localAudio.deviceId =
              "default";
          }
          logger.log(
    ...
    APP.UI.addListener(UIEvents.TOGGLE_AUDIO_ONLY, (audioOnly) => {
      ...
      const isTrackInRedux = Boolean(
        tracks.find(
          (track) => track.jitsiTrack && track.jitsiTrack.getType() === "video"
        )
      );
    ...
    APP.UI.addListener(
      UIEvents.TOGGLE_SCREENSHARING,
      this.toggleScreenSharing.bind(this)
    );
  ...
  _updateVideoDeviceId() {
    if (this.localVideo && this.localVideo.videoType === "camera") {
      APP.store.dispatch(
        updateSettings({
  ...
  _onDeviceListChanged(devices) {
    const oldDevices = APP.store.getState()["features/base/devices"]
      .availableDevices;

    APP.store.dispatch(updateDeviceList(devices));

    const newDevices = mediaDeviceHelper.getNewMediaDevicesAfterDeviceListChanged(
      devices,
      this.isSharingScreen,
      this.localVideo,
      this.localAudio
    ...
    const newAvailDevices = APP.store.getState()["features/base/devices"]
      .availableDevices;
    let newAudioDevices = [];
    let oldAudioDevices = [];
    ...
    if (requestedInput.video && this.isLocalVideoMuted()) {
      ...
      APP.store.dispatch(
        updateSettings({
          // Update the current selected camera for the device selection dialog.
      ...
      mediaDeviceHelper
        ...
        .then((tracks) => {
          ...
          const muteSyncPromises = tracks.map((track) => {
            if (
              (track.isVideoTrack() && videoWasMuted) ||
          ...
          return Promise.all(muteSyncPromises).then(() =>
            Promise.all(
              Object.keys(requestedInput).map((mediaType) => {
                if (requestedInput[mediaType]) {
                  ...
                  return useStream(
                    tracks.find((track) => track.getType() === mediaType) ||
                      null
                  ).then(() => {
                    if (hasDefaultMicChanged) {
                      // workaround for the default device to be shown as selected in the
        ...
        .then(() => {
          ...
          if (audioWasMuted) {
            sendAnalytics(
              createTrackMutedEvent("audio", "device list changed")
            );
          ...
          if (!this.isSharingScreen && videoWasMuted) {
            sendAnalytics(
              createTrackMutedEvent("video", "device list changed")
            );
  ...
  updateAudioIconEnabled() {
    const audioMediaDevices = APP.store.getState()["features/base/devices"]
  ...
  updateVideoIconEnabled() {
    const videoMediaDevices = APP.store.getState()["features/base/devices"]
      .availableDevices.videoInput;
    const videoDeviceCount = videoMediaDevices ? videoMediaDevices.length : 0;
  ...
  hangup(requestFeedback = false) {
    ...
    Promise.all([requestFeedbackPromise, this.leaveRoomAndDisconnect()]).then(
      (values) => {
  ...
  submitFeedback(score = -1, message = "") {
    if (score === -1 || (score >= 1 && score <= 5)) {
      APP.store.dispatch(submitFeedback(score, message));
    }
  },
