...
export function createDesiredLocalTracks(...desiredTypes) {
    return (dispatch, getState) => {
        ...
        const availableTypes = getLocalTracks(
            state["features/base/tracks"],
            /* includePending */ true
        ).map((t) => t.mediaType);

        // We need to create the desired tracks which are not already available.
        const createTypes = desiredTypes.filter(
            (type) => availableTypes.indexOf(type) === -1
        );
...
export function createLocalTracksA(options = {}) {
    return (dispatch, getState) => {
        const devices = options.devices || [MEDIA_TYPE.AUDIO, MEDIA_TYPE.VIDEO];
        const store = {
            dispatch,
            getState,
        };
            ...
            const gumProcess = createLocalTracksF(
                ...
                (localTracks) => {
                    ...
                    if (gumProcess.canceled) {
                        return _disposeTracks(localTracks).then(() =>
        ...
        _cancelGUMProcesses(getState).then(() =>
            dispatch(
                _disposeAndRemoveTracks(
                    getState()
                        ["features/base/tracks"].filter((t) => t.local)
                        .map((t) => t.jitsiTrack)
                )
            )
        );
...
export function showNoDataFromSourceVideoError(jitsiTrack) {
    return async (dispatch, getState) => {
        ...
        if (track.isReceivingData) {
            ...
            const notificationAction = await dispatch(
    ...
    return async (dispatch, getState) => {
        ...
        if (local) {
            ...
            track.on(JitsiTrackEvents.NO_DATA_FROM_SOURCE, () =>
                dispatch(noDataFromSource({ jitsiTrack: track }))
            );
            if (!isReceivingData) {
                if (mediaType === MEDIA_TYPE.AUDIO) {
                    ...
                    dispatch(
                        setNoSrcDataNotificationUid(notificationAction.uid)
                    );
                    ...
                    const timeout = setTimeout(
                        () => dispatch(showNoDataFromSourceVideoError(track)),
                        5000
                    );
 ...
 * }}
 */
export function trackNoDataFromSourceNotificationInfoChanged(
    track,
    noDataFromSourceNotificationInfo
) {
    return {
...
export function toggleCamera() {
    return async (dispatch, getState) => {
        ...
        dispatch(
            updateSettings({
                localFlipX: targetFacingMode === CAMERA_FACING_MODE.USER,
            })
        );

        const newVideoTrack = await createLocalTrack("video", null, null, {
