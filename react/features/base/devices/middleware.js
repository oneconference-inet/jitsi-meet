...
import {
    ...
    replaceVideoTrackById,
    setDeviceStatusWarning,
} from "../../prejoin/actions";
    ...
    switch (action.type) {
        ...
        case NOTIFY_CAMERA_ERROR: {
            ...
            const cameraErrorMsg =
                ...
                JITSI_TRACK_ERROR_TO_MESSAGE_KEY_MAP.camera[
                    JitsiTrackErrors.GENERAL
                ];
            const additionalCameraErrorMsg = cameraJitsiTrackErrorMsg
                ? null
                : message;
            const titleKey =
                name === JitsiTrackErrors.PERMISSION_DENIED
                    ? "deviceError.cameraPermission"
        ...
        case NOTIFY_MIC_ERROR: {
            ...
            const micErrorMsg =
                ...
                JITSI_TRACK_ERROR_TO_MESSAGE_KEY_MAP.microphone[
                    JitsiTrackErrors.GENERAL
                ];
            const additionalMicErrorMsg = micJitsiTrackErrorMsg
                ? null
                : message;
            const titleKey =
                name === JitsiTrackErrors.PERMISSION_DENIED
...
function _checkAndNotifyForNewDevice(store, newDevices, oldDevices) {
    ...
    const onlyNewDevices = newDevices.filter(
        (nDevice) =>
            !oldDevices.find((device) => device.deviceId === nDevice.deviceId)
    );
    ...
    Object.values(devicesGroupBy).forEach((devicesArray) => {
        ...
        dispatch(
            showNotification({
                ...
                customActionHandler: _useDevice.bind(
                    undefined,
                    store,
                    devicesArray
                ),
